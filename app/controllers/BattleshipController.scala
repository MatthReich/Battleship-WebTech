...
class BattleshipController @Inject() (
  ...
  def playAgain(): Action[AnyContent] = SecuredAction.async { implicit request: SecuredRequest[EnvType, AnyContent] =>
    authInfoRepository.find[GoogleTotpInfo](request.identity.loginInfo).map { totpInfoOpt =>
      Ok(game(request.identity, totpInfoOpt)).withHeaders("Acces-Control-Allow-Origin" -> "http://localhost:8080")
    }
  }

  def aboutpage = SecuredAction.async { implicit request: SecuredRequest[EnvType, AnyContent] =>
    authInfoRepository.find[GoogleTotpInfo](request.identity.loginInfo).map { totpInfoOpt =>
      Ok(about(request.identity, totpInfoOpt)).withHeaders("Acces-Control-Allow-Origin" -> "http://localhost:8080")
    }
  }
  ...
  def setPlayer(): Action[AnyContent] = Action { implicit request =>
    ...
    selection.map { args =>
      ...
      Ok(game(null, null)).withHeaders("Acces-Control-Allow-Origin" -> "http://localhost:8080")
